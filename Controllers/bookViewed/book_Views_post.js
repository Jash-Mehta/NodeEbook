...
async function hasUserViewedBook(userId, bookId, promisePool) {
    try {
        ...
        if (rows.length > 0) {
            if (rows[0].count > 0) {
                return "You have already visited this book."; // Custom message if the user has already viewed the book
            } else {
...
async function incrementViewCount(bookId, promisePool, res) {
    try {
        // Assuming you're using a MySQL database
        const query = 'INSERT INTO bookViews (book_id, views) VALUES (?, 1) ON DUPLICATE KEY UPDATE views = views + 1, updated_at = CURRENT_TIMESTAMP';
        const [result, fields] = await promisePool.query(query, [bookId]);
